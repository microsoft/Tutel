...
torch::Tensor warp_deepseek_r1_attn_f16xf8_block_scal(
  ...
  if (val_cache.numel() <= 1) {
    ...
    auto it2 = wkv_b_.find(kv_b_proj_scal.data_ptr());
    if (it2 == wkv_b_.end()) {
  ...
  int64_t pos
    ...
    for (int l = 0; l < rms_att_ws.size(); ++l) {
      ...
      if (l < weight_gate_ups.size()) {
        ...
        antares::ops::call("deepseek_r1_sigmoid_top_k_routed_scaled_f32", {logits_bf16.view({samples, logits_bf16.size(-1)}), gate_biases[l - 3], score_weight, topk_exp_id}, {}, false, 0, 3);
