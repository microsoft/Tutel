...
class FusedExpertsNetwork(torch.nn.Module):
    ...
    def reset_parameters(self):
        with torch.no_grad():
            import math
            fc1_stdv, fc2_stdv = 1. / math.sqrt(self.batched_fc1_w.size(-1)), 1. / math.sqrt(self.batched_fc2_w.size(-1))
            for i in range(self.batched_fc1_w.size(0)):
                self.batched_fc1_w[i] = (torch.rand([self.model_dim, self.hidden_size], dtype=torch.float32, device=self.batched_fc1_w.device) * (fc1_stdv + fc1_stdv) - fc1_stdv).to(self.batched_fc1_w.dtype)
